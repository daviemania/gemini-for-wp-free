...
║    /tools    - Show available WordPress tools                 ║
║    /mcp      - Enable/disable MCP tools                       ║
...
rl.on("line", async (input) => {
    ...
    if (userInput === "/models") {
        ...
        if (models.cloud.available.length > 0) {
            ...
            if (models.cloud.recommended) {
                console.log(
                    `   🌟 Recommended: ${models.cloud.recommended.model}`,
                );
        ...
        if (models.local.available.length > 0) {
            ...
            if (models.local.recommended) {
                console.log(
                    `   🌟 Recommended: ${models.local.recommended.model}`,
                );
    ...
    if (userInput === "/tools") {
        ...
        console.log(`MCP Status: ${mcpEnabled ? "✅ ENABLED" : "❌ DISABLED"}`);
        console.log("Enable with: /mcp on\n");
        rl.prompt();
        return;
    }

    if (userInput.startsWith("/mcp")) {
        const action = userInput.split(" ")[1];
        if (action === "on") {
            mcpEnabled = true;
            console.log(
                "✅ MCP tools ENABLED - Ollama can now access WordPress\n",
            );
            console.log(
                'Try: "List my WordPress plugins" or "How many posts do I have?"\n',
            );
        } else if (action === "off") {
            mcpEnabled = false;
            ...
            console.log("Usage: /mcp on  or  /mcp off\n");
        }
        rl.prompt();
        return;
    }
    ...
    if (userInput === "/stats") {
        ...
        console.log(
            `   Requests: ${stats.cloud.requests} (${stats.cloud.percentage})`,
        );
        ...
        console.log("💻 Local Models:");
        console.log(
            `   Requests: ${stats.local.requests} (${stats.local.percentage})`,
        );
...
Commands:
  ...
  /tools   - Show available WordPress MCP tools
  /mcp     - Enable/disable MCP tool access
...
Tips:
  ...
  • Enable /mcp on to let Ollama access WordPress
...
`);
    ...
    try {
        ...
        // Add tools if MCP is enabled
        if (mcpEnabled) {
            chatOptions.tools = MCP_TOOLS;
        }

        const result = await ollama.chat(userInput, chatOptions);
        ...
        if (result.success) {
            ...
            if (toolCalls.length > 0) {
                console.log(
                    `🔧 Using tools: ${toolCalls.map((t) => t.function?.name || "unknown").join(", ")}\n`,
                );
                ...
                for (const toolCall of toolCalls) {
                    ...
                    if (!functionName) {
                        console.log(
                            `   ✗ Invalid tool call (no function name)\n`,
                        );
                        continue;
                    }
                ...
                const finalMessages = [
                    ...conversationHistory,
                    {
                        role: "user",
                        content:
                            "Based on the tool results above, provide a clear and natural response.",
                    },
                ];

                const finalResult = await ollama.chat(
                    "Provide response based on tool results",
                    { messages: finalMessages },
                );

                if (finalResult.success) {
                    console.log(
                        `🦙 ${ollama.activeModel}: ${finalResult.response}\n`,
                    );
                    console.log(
                        `   [${finalResult.modelType} • ${finalResult.responseTime}ms • ${finalResult.tokenCount} tokens]\n`,
                    );
                    conversationHistory.push({
                        role: "assistant",
                    ...
                    console.log(
                        `❌ Error getting final response: ${finalResult.error}\n`,
                    );
                }
            } else {
                ...
                console.log(
                    `   [${result.modelType} • ${result.responseTime}ms • ${result.tokenCount} tokens]\n`,
                );
            ...
            console.log(
                "Try /models to see available models or /switch to change model\n",
            );
        }
    } catch (error) {
        console.error(`\n❌ Error: ${error.message}\n`);

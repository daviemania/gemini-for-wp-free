...
class OllamaManager {
            ...
            for (const tier of ['huge', 'large', 'fast']) {
                for (const model of this.models.cloud[tier]) {
                    if (this.available.cloud.includes(model)) {
                        ...
                        console.log(`✅ Selected cloud model: ${model} (${tier})`);
                        return;
                    }
                }
            }
            ...
            for (const tier of ['medium', 'small', 'tiny']) {
                for (const model of this.models.local[tier]) {
                    if (this.available.local.includes(model)) {
                        ...
                        if (freeMem > memNeeded + 1000) { // 1GB safety margin
                            this.activeModel = model;
                            this.modelType = 'local';
                            console.log(`✅ Selected local model: ${model} (${tier}, needs ~${memNeeded}MB)`);
                            return;
                        }
                    }
                }
    ...
    getFreeMemory() {
        try {
            return parseInt(execSync("free -m | awk 'NR==2{print $7}'").toString().trim());
        } catch (error) {
    ...
    async chat(message, options = {}) {
        ...
        try {
            ...
            const ollamaTools = tools ? this.convertToolsToOllamaFormat(tools) : undefined;
            ...
            this.stats[modelType].avgTime = 
                (this.stats[modelType].avgTime * (this.stats[modelType].requests - 1) + responseTime) / 
                this.stats[modelType].requests;
            ...
            if (toolCalls.length > 0 && mcpClient) {
                ...
                executedTools = await this.executeToolCalls(toolCalls, mcpClient);
            }
    ...
    async executeToolCalls(toolCalls, mcpClient) {
        ...
        for (const call of toolCalls) {
            try {
                console.log(`   → ${call.function.name}(${JSON.stringify(call.function.arguments).substring(0, 50)}...)`);
                ...
                if (toolResult.data) {
                    if (Array.isArray(toolResult.data)) {
                        console.log(`   ✓ Returned ${toolResult.data.length} items`);
                    } else {
                        console.log(`   ✓ Success`);
                    }
                }
    ...
    getRecommendedCloudModel() {
        ...
        if (gpt120) return { model: gpt120, reason: 'Best balance of speed and quality' };
        
        // Next: DeepSeek for reasoning
        const deepseek = cloudAvailable.find(m => m.startsWith('deepseek'));
        if (deepseek) return { model: deepseek, reason: 'Best for complex reasoning' };
        
        return { model: cloudAvailable[0], reason: 'First available cloud model' };
    }
    ...
    getRecommendedLocalModel() {
        ...
        if (freeMem > 3000) {
            const medium = localAvailable.find(m => 
                m.includes('3b') || m.includes('phi3.5')
            );
            if (medium) return { model: medium, reason: 'Good quality, fits in memory' };
        }
        
        if (freeMem > 1500) {
            const small = localAvailable.find(m => m.includes('1b'));
            if (small) return { model: small, reason: 'Fast and memory-efficient' };
        }
        
        // Fallback to smallest
        const tiny = localAvailable.find(m => m.includes('0.5b') || m.includes('tiny'));
    ...
    getStats() {
        const totalRequests = this.stats.cloud.requests + this.stats.local.requests;
        ...
        return {
            ...
            cloud: {
                ...
                percentage: totalRequests > 0 
                    ? ((this.stats.cloud.requests / totalRequests) * 100).toFixed(1) + '%'
                    : '0%',
            ...
            local: {
                ...this.stats.local,
                percentage: totalRequests > 0 
                    ? ((this.stats.local.requests / totalRequests) * 100).toFixed(1) + '%'
                    : '0%',
